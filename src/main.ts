    ...
    if (spotlightWindow) {
      try {
        ...
        spotlightWindow.webContents.executeJavaScript(
          "window.windowAPI && window.windowAPI.appModeEnd && window.windowAPI.appModeEnd()"
        );
      } catch (e) {
...
app.whenReady().then(async () => {
  ...
  const mainWindowHotkeyRegistered = globalShortcut.register(
    mainWindowHotkey,
    () => {
      if (mainWindow) {
        if (mainWindow.isVisible()) {
  ...
  const triggerAppSwitchInit = () => {
    ...
    setTimeout(() => {
      if (spotlightWindow) {
        ...
        spotlightWindow.focus();
        spotlightWindow.webContents.executeJavaScript(
          "window.initAppModeWithNext && window.initAppModeWithNext()"
        );
      }
    }, 100);
  };
    ...
    if (windowManager && mainWindow) {
      try {
        ...
        if (currentActiveApp !== lastKnownActiveApp) {
          console.log(
            `🔄 Active app changed: ${lastKnownActiveApp} → ${currentActiveApp}`
          );
  ...
  ipcMain.handle("app-mode-start", async () => {
    try {
      if (globalShortcut.isRegistered(appSwitchHotkey)) {
        ...
        console.log(
          `Global hotkey temporarily unregistered: ${appSwitchHotkey}`
        );
      }
    } catch (e) {
      console.error(
        "Error unregistering app switch hotkey on app-mode-start:",
        e
      );
    }
  });
  ...
  ipcMain.handle("app-mode-end", async () => {
    try {
      ...
      console.error(
        "Error re-registering app switch hotkey on app-mode-end:",
        e
      );
    }
  });
  ...
  ipcMain.handle(
    ...
    async (_, name: string, description?: string) => {
      ...
      console.log(
        `Preset saved: ${preset.name} with ${windows.length} windows`
      );
      return preset;
    }
  );
  ...
  ipcMain.handle("delete-preset", async (_, presetId: string) => {
    ...
    console.log(
      `Preset ${presetId} deletion: ${deleted ? "success" : "failed"}`
    );
    return deleted;
  });
  ...
  ipcMain.handle(
    "update-preset",
    async (_, presetId: string, name?: string, description?: string) => {
      const updates: any = {};
      ...
      console.log(
        `Preset ${presetId} update: ${updated ? "success" : "failed"}`
      );
      return updated;
    }
  );
  ...
  ipcMain.handle("suggest-apps-for-task", async (_, userPrompt: string) => {
    ...
    // Claudeでアプリを提案
    const suggestions = await claudeService.suggestAppsForTask(
      userPrompt,
      applicationGraph
    );
    console.log(
      `Suggested apps - High: ${suggestions.highConfidence.length}, Low: ${suggestions.lowConfidence.length}`
    );
  ...
  ipcMain.handle(
    "open-apps-for-task",
    async (_, appNames: string[], taskName: string) => {
      console.log(`Opening ${appNames.length} apps for task: ${taskName}`);

      // 選択されたアプリを並列で開く
      const launchPromises = appNames.map((appName) =>
        appScanner.launchApp(appName)
      );
      await Promise.all(launchPromises);
      ...
      console.log(
        `Auto-saved preset: ${preset.name} with ${windows.length} windows`
      );
      return preset;
    }
  );
  ...
  ipcMain.handle("quit-recommended-app", async (_, appName: string) => {
    try {
...
async function updateAnalysisInterval() {
  ...
  if (settings.analysisInterval > 0) {
    ...
    console.log(
      `⏰ Analysis interval set to ${settings.analysisInterval / 1000}s`
    );
  } else {
...
async function performAIAnalysis() {
  ...
  try {
    ...
    console.log(
      `📊 Analyzing ${focusStats.length} apps, ${processes.length} processes`
    );
    ...
    console.log(
      `Found ${focusAnalysis.distractingApps.length} distracting apps`
    );
    ...
    console.log(
      `Found ${resourceAnalysis.heavyResourceApps.length} heavy resource apps`
    );
    ...
    console.log(
      `✅ Analysis complete: ${appsToClose.length} apps recommended to close`
    );
    console.log(
      `📈 System health score: ${recommendations.systemHealthScore}/100`
    );
    ...
    if (appsToClose.length > 0) {
      ...
      appsToClose.forEach((app) => {
        console.log(
          `  - ${app.appName} (${app.priority}): ${app.expectedBenefit}`
        );
        console.log(
          `    Reasons: ${
            Array.isArray(app.reasons)
              ? app.reasons.join(", ")
              : "No reasons provided"
          }`
        );
