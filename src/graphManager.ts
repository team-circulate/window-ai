...
export class GraphManager {
  ...
  constructor() {
    ...
    this.graphPath = path.join(
      ...
      "application_graph.json"
    );
    this.onboardingPath = path.join(
      app.getPath("userData"),
  ...
  private saveOnboardingStatus(): void {
    try {
      fs.writeFileSync(
        this.onboardingPath,
        JSON.stringify(this.onboardingStatus, null, 2)
      );
    } catch (error) {
  ...
  public addApplications(
    apps: Array<{ name: string; observations: string[] }>
  ): void {
    ...
    return this.graph
      .filter(
        (node): node is EntityNode =>
          node.type === "entity" && node.entityType === "Application"
      )
      .map((app) => ({
        name: app.name,
        observations: app.observations,
